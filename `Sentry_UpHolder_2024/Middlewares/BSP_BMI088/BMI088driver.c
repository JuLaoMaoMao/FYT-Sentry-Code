...
static pt1Filter_t gyroFilter[3];   //滤波器参数
...
uint8_t BMI088_init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
  ...
  for(axis=0; axis<3; axis++) {
    biquadFilterInitLPF(&accFilter[axis], 50, 800);
    pt1FilterInit(&gyroFilter[axis], 200,5e-4);    
  }
    return error;
}
...
//    bmi088->Accel[axis] = biquadFilterApply(&accFilter[axis], bmi088->Accel[axis]);
//    bmi088->Gyro[axis] = pt1FilterApply(&gyroFilter[axis], bmi088->Gyro[axis]);    
//  }
